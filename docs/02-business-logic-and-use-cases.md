...
- `category_id` (GPU, CPU, Motherboard, Keyboard, Mouse, Headset, Memory, Storage, Case, PSU, Cooling)
- `brand`, `specs` (JSONB — flexible per category: e.g. GPU has VRAM, CUDA cores; keyboard has switch type)
- `price_cents`, `currency` (USD only in MVP)
- `images` (array of S3 keys)
- `status` (DRAFT, ACTIVE, DISCONTINUED)

### Category
Owned by Catalog. Simple tree, one level deep for MVP.

### User
Owned by Identity (external). Services only know the user by `user_id` (the JWT `sub` claim). No PII stored in our services beyond what's necessary for orders.

### Cart
Lives in Redis. Keyed by session ID for guests, user ID for logged-in users.
- List of `{product_id, quantity, unit_price_cents_at_add_time}`
- TTL: 7 days for guest, 30 days for logged-in

### Stock
Owned by Inventory.
- `product_id`, `available`, `reserved`, `version` (for optimistic locking)
- `reservations` table: `{reservation_id, order_id, product_id, quantity, expires_at, state}`

### Order
Owned by Orders.
- `id`, `user_id`, `status`, `total_cents`, `currency`
- `status`: PENDING → RESERVING → PAYING → CONFIRMED / FAILED
- `line_items`: `{product_id, quantity, unit_price_cents, name_snapshot}`
- `shipping_address`, `billing_address`
- `payment_intent_id` (Stripe)
- `idempotency_key`
- `outbox` (separate table for event publishing)

### Payment
Phase 2 entity. In MVP, represented as fields on the Order plus a `payment_events` table (audit trail of Stripe webhook calls).
...
4. Outbox poller publishes OrderPlaced to SNS.
...
10. Stripe webhook fires → Lambda → SNS → Orders consumer.
...
sequenceDiagram
    ...
    participant Stripe
    participant SNS_SQS as SNS / SQS
    ...
    Note over Orders: Outbox poller publishes OrderPlaced → SNS
    ...
    Stripe->>SNS_SQS: payment_intent.succeeded webhook → Lambda → SNS → SQS
    SNS_SQS->>Orders: consume PaymentCaptured

    Note over Orders: Saga transitions → CONFIRMED
    Orders->>Orders: write outbox: OrderConfirmed

    SNS_SQS->>Inventory: consume OrderConfirmed
