...
This is a learning project built to production standards. The domain (**GG Gaming** — a gaming hardware e-commerce store at `gaming.gg`) is a vehicle; the real goal is hands-on mastery of distributed systems, observability, Kubernetes, and cloud infrastructure. The product will work end-to-end, but "ship a viable business" is explicitly not the main goal — "exercise every meaningful pattern in a modern backend stack" is.
...
3. **Infrastructure as Code and Kubernetes** — Terraform/Terragrunt, EKS, GitOps
...
- Three backend services (Catalog, Orders, Inventory) deployed to Kubernetes
- Full observability: distributed tracing, metrics, structured logs, dashboards
- CI/CD: GitHub Actions → container registry → ArgoCD → Kubernetes
- Infrastructure as Code covering the full stack: VPC, cluster, databases, queues, IAM
...
- **Kafka** — SNS/SQS is sufficient for MVP (see ADR-004)
...
4. Killing a service pod mid-saga results in the saga completing or compensating correctly after restart (no stuck orders, no double charges).
5. All infrastructure is defined in Terraform/Terragrunt and can be torn down and rebuilt from scratch.
6. CI pipeline runs on every PR; merge to main triggers deploy to a dev environment via ArgoCD.
7. The whole stack can be spun up, demoed, and torn down without manual AWS console clicks.
