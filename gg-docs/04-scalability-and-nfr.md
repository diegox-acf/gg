...
- Kafka (in-flight messages, committed offsets)
- Local filesystem (images — MinIO in Phase 2)

This means: container restarts are safe, and the saga resumes from its last committed state.

## Event-driven back pressure

- Kafka consumer lag is a monitored metric. Alert fires when consumer lag exceeds 1000 messages.
- DLQ topic messages trigger immediate alert; never ignored.
